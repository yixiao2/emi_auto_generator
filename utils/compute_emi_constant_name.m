...
if (~isfield(var,'emi_constant_name'))
    tmp = findstr(var.elm_name,'_');
    if(~isfield(var,'emi_constants_prefix'))
        emi_constant_name = [constant_prefix '_' upper(var.elm_name(1:tmp(end)-1))];
    else
        emi_constant_name = [upper(var.emi_constants_prefix) '_' upper(var.elm_name(1:tmp(end)-1))];
    end
else
    if(~isfield(var,'emi_constants_prefix'))
